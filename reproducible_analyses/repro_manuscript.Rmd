...
RUN_MODEL <- FALSE # change me to rerun models

# Helper: run cfa() or sem(), capture warnings/messages, save both fit + log
run_lavaan <- function(FUN, ..., save_path) {
  ...
  log <- list(warnings = captured_warnings, messages = captured_messages)
  saveRDS(fit, save_path)
  saveRDS(log, paste0(save_path, ".warnings.rds"))

  if (length(captured_warnings) > 0) {
    cat("--- Warnings during cfa()/sem() ---\n")
    for (w in captured_warnings) cat("WARNING:", w, "\n")
  }
  if (length(captured_messages) > 0) {
    cat("--- Messages during cfa()/sem() ---\n")
    for (m in captured_messages) cat("MESSAGE:", m, "\n")
  }

  invisible(fit)
}

# Helper: summarise a lavaan fit object and capture + print any warnings/messages
# If rds_path is supplied, also reads the companion .warnings.rds saved by run_lavaan()
lavaan_summary <- function(fit, label = NULL, rds_path = NULL, ...) {
  if (!is.null(label)) cat("\n===", label, "===\n")

  # Print warnings/messages captured at fit time (saved by run_lavaan)
  if (!is.null(rds_path)) {
    log_path <- paste0(rds_path, ".warnings.rds")
    if (file.exists(log_path)) {
      log <- readRDS(log_path)
      if (length(log$warnings) > 0) {
        cat("--- Warnings from cfa()/sem() ---\n")
        for (w in log$warnings) cat("WARNING:", w, "\n")
      }
      if (length(log$messages) > 0) {
        cat("--- Messages from cfa()/sem() ---\n")
        for (m in log$messages) cat("MESSAGE:", m, "\n")
      }
      if (length(log$warnings) == 0 && length(log$messages) == 0)
        cat("No warnings or messages captured during cfa()/sem()\n")
    } else {
      cat("(No warnings log found — rerun with RUN_MODEL <- TRUE to capture)\n")
    }
  }
  
  # Collect warnings and messages emitted during summary()
  captured_warnings  <- character(0)
  captured_messages  <- character(0)
  ...
  # Run lavaan's built-in post-estimation checks explicitly
  cat("\n--- lavaan diagnostic checks ---\n")
  cat("Converged          :", lavInspect(fit, "converged"), "\n")
  cat("Post-check passed  :", lavInspect(fit, "post.check"), "\n")
  
  # Negative residual variances (Heywood cases)
  # lavInspect returns a list for multilevel models (one matrix per level)
  theta_raw <- lavInspect(fit, "theta")
  if (is.list(theta_raw)) {
    theta_diags <- lapply(names(theta_raw), function(lvl) {
      d <- diag(theta_raw[[lvl]])
      if (any(d < 0)) cat(sprintf("NEGATIVE RESIDUAL VARIANCES (Heywood) at level '%s':\n", lvl))
      d[d < 0]
    })
    neg_resid <- unlist(theta_diags)
    if (length(neg_resid) == 0) cat("Negative residual variances : none\n")
  } else {
    theta_diag <- diag(theta_raw)
    neg_resid  <- theta_diag[theta_diag < 0]
    if (length(neg_resid) > 0) {
      cat("NEGATIVE RESIDUAL VARIANCES (Heywood):\n")
      print(neg_resid)
    } else {
      cat("Negative residual variances : none\n")
    }
  }
  
  # Non-positive-definite latent covariance matrix
  # Also a list for multilevel models
  psi_raw <- lavInspect(fit, "cov.lv")
  psi_list <- if (is.list(psi_raw)) psi_raw else list(single = psi_raw)
  for (lvl in names(psi_list)) {
    psi <- psi_list[[lvl]]
    if (!is.null(psi) && length(psi) > 0) {
      eigs <- tryCatch(eigen(psi, only.values = TRUE)$values, error = function(e) NULL)
      lbl  <- if (length(psi_list) > 1) sprintf("psi [level '%s']", lvl) else "Latent cov matrix (psi)"
      if (!is.null(eigs) && any(eigs <= 0)) {
        cat(sprintf("WARNING: %s is NOT positive definite. Eigenvalues:\n", lbl))
        print(round(eigs, 6))
      } else {
        cat(sprintf("%s : positive definite\n", lbl))
      }
    }
  }
  
  # Print any captured warnings / messages
  if (length(captured_warnings) > 0) {
    cat("\n--- Warnings captured during summary() ---\n")
    for (w in captured_warnings) cat("WARNING:", w, "\n")
  }
  if (length(captured_messages) > 0) {
    cat("\n--- Messages captured during summary() ---\n")
    for (m in captured_messages) cat("MESSAGE:", m, "\n")
  }
...
country_n <- dt4 %>% 
  select(country_res_full) %>% 
  group_by(country_res_full) %>% 
  summarize(n = n())

write.csv(country_n, "output/country_n_app.csv")
```
...
write.csv(all_descriptives, "output/scale_indices_descriptives.csv", row.names = FALSE)
...
lavaan_summary(fit, label = "Pilot2 CFA Norms", fit.measures=TRUE, standardized=TRUE)
...
'
fit <- cfa(dhf6_pilot2, data=dt_pilot2)
lavaan_summary(fit, label = "Pilot2 CFA Endorsement", fit.measures=TRUE, standardized=TRUE)
...
```{r individual-hierarchical-test1-updated, eval = RUN_MODEL}
...
hierarchical.individual.test1.fit <- run_lavaan(cfa,model = hierarchical.model.test1,
                    data = dt4,
                    ...
                    # em.fx.tol = 1e-08, 
                    # em.dx.tol = 1e-04,
                    save_path = "output/hierarchical.individual.test1.fit.rds"
)
```
...
lavaan_summary(hierarchical.individual.test1.fit, rds_path = "output/hierarchical.individual.test1.fit.rds", label = "Endorsement Hierarchical Individual L1",
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)
...
```{r individual-hierarchical-test2-original, eval = RUN_MODEL}
...
hierarchical.individual.test2.fit <- run_lavaan(cfa,model = hierarchical.model.test2,
                    data = dt4,
                    cluster = "country_res_full",
                    # options to consider
                    #optim.method = "em", 
                    ...
                    #em.fx.tol = 1e-08, 
                    #em.dx.tol = 1e-04,
                    save_path = "output/hierarchical.individual.test2.fit"
)
```
...
lavaan_summary(hierarchical.individual.test2.fit, rds_path = "output/hierarchical.individual.test2.fit", label = "Endorsement Hierarchical Individual L2",
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)
...
```{r individual-threefactor-test1, eval = RUN_MODEL}
...
threefactor.individual.test1.fit <- run_lavaan(cfa,model = threefactor.model.test1,
                    data = dt4,
                    cluster = "country_res_full",
                    # options to consider
                    optim.method = "em",
                    ...
                    #em.fx.tol = 1e-08, 
                    #em.dx.tol = 1e-04,
                    save_path = "output/threefactor.individual.test1.fit"
)
```
...
lavaan_summary(threefactor.individual.test1.fit, rds_path = "output/threefactor.individual.test1.fit", label = "Endorsement 3-Factor Individual L1",
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)
...
```{r individual-threefactor-test2, eval = RUN_MODEL}
...
threefactor.individual.test2.fit <- run_lavaan(cfa,model = threefactor.model.test2,
                    data = dt4,
                    cluster = "country_res_full",
                    # options to consider
                    #optim.method = "em", 
                    ...
                    #em.fx.tol = 1e-08, 
                    #em.dx.tol = 1e-04,
                    save_path = "output/threefactor.individual.test2.fit"
)
```
...
lavaan_summary(threefactor.individual.test2.fit, rds_path = "output/threefactor.individual.test2.fit", label = "Endorsement 3-Factor Individual L2",
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)
...
```{r individual-sixfactor-test1, eval = RUN_MODEL}
...
sixfactor.individual.test1.fit <- run_lavaan(cfa,model = sixfactor.model.test1,
                    data = dt4,
                    cluster = "country_res_full",
                    # options to consider
                    optim.method = "em", 
                    ...
                    #em.fx.tol = 1e-08, 
                    #em.dx.tol = 1e-04,
                    save_path = "output/sixfactor.individual.test1.fit"
)
```
...
lavaan_summary(sixfactor.individual.test1.fit, rds_path = "output/sixfactor.individual.test1.fit", label = "Endorsement 6-Factor Individual L1",
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)
...
```{r individual-sixfactor-test2, eval = RUN_MODEL}
...
sixfactor.individual.test2.fit <- run_lavaan(cfa,model = sixfactor.model.test2,
                    data = dt4,
                    cluster = "country_res_full",
                    # options to consider
                    #optim.method = "em", 
                    ...
                    #em.fx.tol = 1e-08, 
                    #em.dx.tol = 1e-04,
                    save_path = "output/sixfactor.individual.test2.fit"
)
```
...
lavaan_summary(sixfactor.individual.test2.fit, rds_path = "output/sixfactor.individual.test2.fit", label = "Endorsement 6-Factor Individual L2",
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)
...
```{r individual-hierarchical-testc-updated, eval = RUN_MODEL}
...
hierarchical.model.testc.fit <- run_lavaan(cfa,model = hierarchical.model.testc,
                    data = dt4,
                    cluster = "country_res_full",
                    # options to consider
                    # optim.method = "em"
                    ...
                    # em.fx.tol = 1e-08, 
                    # em.dx.tol = 1e-04,
                    save_path = "output/hierarchical.model.testc.fit"
)
```
...
lavaan_summary(hierarchical.model.testc.fit, rds_path = "output/hierarchical.model.testc.fit", label = "Endorsement Hierarchical Combined",
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)
...
```{r individual-sixfactor-testc, eval = RUN_MODEL}
...
sixfactor.individual.testc.fit <- run_lavaan(cfa,model = sixfactor.model.testc,
                    data = dt4,
                    cluster = "country_res_full",
                    # options to consider
                    #optim.method = "em", 
                    ...
                    #em.fx.tol = 1e-08, 
                    #em.dx.tol = 1e-04,
                    save_path = "output/sixfactor.individual.testc.fit"
)
```
...
lavaan_summary(sixfactor.individual.testc.fit, rds_path = "output/sixfactor.individual.testc.fit", label = "Endorsement 6-Factor Combined",
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)
...
```{r individual-threefactor-testc, eval = RUN_MODEL}
...
threefactor.individual.testc.fit <- run_lavaan(cfa,model = threefactor.model.testc,
                    data = dt4,
                    cluster = "country_res_full",
                    # options to consider
                    #optim.method = "em", 
                    ...
                    #em.fx.tol = 1e-08, 
                    #em.dx.tol = 1e-04,
                    save_path = "output/threefactor.individual.testc.fit"
)
```
...
lavaan_summary(threefactor.individual.testc.fit, rds_path = "output/threefactor.individual.testc.fit", label = "Endorsement 3-Factor Combined",
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)
...
```{r individual-six3factor-testc, eval = RUN_MODEL}
...
six3factor.individual.testc.fit <- run_lavaan(cfa,model = six3factor.model.testc,
                    data = dt4,
                    cluster = "country_res_full",
                    # options to consider
                    #optim.method = "em", 
                    ...
                    #em.fx.tol = 1e-08, 
                    #em.dx.tol = 1e-04,
                    save_path = "output/six3factor.individual.testc.fit"
)
```
...
lavaan_summary(six3factor.individual.testc.fit, rds_path = "output/six3factor.individual.testc.fit", label = "Endorsement 6+3-Factor Combined",
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)
...
```{r individual-six3factor-testc-no13, eval = RUN_MODEL}
...
six3factor.model.testc.no13.fit <- run_lavaan(cfa,model = six3factor.model.testc.no13,
                    data = dt4,
                    cluster = "country_res_full",
                    # options to consider
                    #optim.method = "em", 
                    ...
                    #em.fx.tol = 1e-08, 
                    #em.dx.tol = 1e-04,
                    save_path = "output/six3factor.model.testc.no13.fit"
)
```
...
lavaan_summary(six3factor.model.testc.no13.fit, rds_path = "output/six3factor.model.testc.no13.fit", label = "Endorsement 6+3-Factor Combined no13",
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)
...
```{r individual-six3factor-testc-no20, eval = RUN_MODEL}
...
six3factor.model.testc.no20.fit <- run_lavaan(cfa,model = six3factor.model.testc.no20,
                    data = dt4,
                    cluster = "country_res_full",
                    # options to consider
                    #optim.method = "em", 
                    ...
                    #em.fx.tol = 1e-08, 
                    #em.dx.tol = 1e-04,
                    save_path = "output/six3factor.model.testc.no20.fit"
)
```
...
lavaan_summary(six3factor.model.testc.no20.fit, rds_path = "output/six3factor.model.testc.no20.fit", label = "Endorsement 6+3-Factor Combined no13+20",
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)
...
```{r norms-hierarchical-test1, eval = RUN_MODEL}
hierarchical.model.test1 <- 
paste0(' 
Level: 1
...
hierarchical.norms.test1.fit <- run_lavaan(cfa,model = hierarchical.model.test1,
                    data = dt4,
                    cluster = "country_res_full",
                    # options to consider
                    optim.method = "em",
                    ...
                    # em.fx.tol = 1e-08, 
                    # em.dx.tol = 1e-04,
                    save_path = "output/hierarchical.norms.test1.fit"
)
```
...
lavaan_summary(hierarchical.norms.test1.fit, rds_path = "output/hierarchical.norms.test1.fit", label = "Norms Hierarchical L1", 
        fit.measures = TRUE, 
        rsquare = TRUE, 
        standardized = TRUE)
...
```{r norms-hierarchical-test2-original, eval = RUN_MODEL}
hierarchical.model.test2 <- 
paste0(' 
Level: 1', 
corr_norm, 
...
hierarchical.norms.test2.fit <- run_lavaan(cfa,model = hierarchical.model.test2,
                    data = dt4,
                    cluster = "country_res_full",
                    # options to consider
                    #optim.method = "em", 
                    ...
                    #em.fx.tol = 1e-08, 
                    #em.dx.tol = 1e-04
                    save_path = "output/hierarchical.norms.test2.fit"
                    )

lavaan_summary(hierarchical.norms.test2.fit, rds_path = "output/hierarchical.norms.test2.fit", label = "Norms Hierarchical L2",
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)
```
...
```{r norms-threefactor-test1, eval = RUN_MODEL}
threefactor.model.test1 <- 
paste0(' 
Level: 1
...
threefactor.norms.test1.fit <- run_lavaan(cfa,model = threefactor.model.test1,
                    data = dt4,
                    cluster = "country_res_full",
                    # options to consider
                    optim.method = "em", 
                    ...
                    em.fx.tol = 1e-08, 
                    em.dx.tol = 1e-04,
                    save_path = "output/threefactor.norms.test1.fit"
)
```
...
lavaan_summary(threefactor.norms.test1.fit, rds_path = "output/threefactor.norms.test1.fit", label = "Norms 3-Factor L1",
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)
...
```{r norms-threefactor-test2, eval = RUN_MODEL}
threefactor.model.test2 <- 
paste0(' 
Level: 1', 
corr_norm, 
...
threefactor.norms.test2.fit <- run_lavaan(cfa,model = threefactor.model.test2,
                    data = dt4,
                    cluster = "country_res_full",
                    # options to consider
                    #optim.method = "em", 
                    ...
                    #em.fx.tol = 1e-08, 
                    #em.dx.tol = 1e-04,
                    save_path = "output/threefactor.norms.test2.fit"
)
```
...
lavaan_summary(threefactor.norms.test2.fit, rds_path = "output/threefactor.norms.test2.fit", label = "Norms 3-Factor L2",
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)
...
```{r norms-sixfactor-test1, eval = RUN_MODEL}
sixfactor.model.test1 <- 
paste0(' 
Level: 1
  D_self =~ dhf_norm_1+dhf_norm_2+dhf_norm_3+dhf_norm_4+dhf_norm_5
...
sixfactor.norms.test1.fit <- run_lavaan(cfa,model = sixfactor.model.test1,
                    data = dt4,
                    cluster = "country_res_full",
                    # options to consider
                    optim.method = "em", 
                    ...
                    em.fx.tol = 1e-08, 
                    em.dx.tol = 1e-04,
                    save_path = "output/sixfactor.norms.test1.fit"
)
```
...
lavaan_summary(sixfactor.norms.test1.fit, rds_path = "output/sixfactor.norms.test1.fit", label = "Norms 6-Factor L1",
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)
...
```{r norms-sixfactor-test2, eval = RUN_MODEL}
sixfactor.model.test2 <- 
paste0(' 
Level: 1', 
corr_norm, 
...
sixfactor.norms.test2.fit <- run_lavaan(cfa,model = sixfactor.model.test2,
                    data = dt4,
                    cluster = "country_res_full",
                    # options to consider
                    #optim.method = "em", 
                    ...
                    #em.fx.tol = 1e-08, 
                    #em.dx.tol = 1e-04,
                    save_path = "output/sixfactor.norms.test2.fit"
)
```
...
lavaan_summary(sixfactor.norms.test2.fit, rds_path = "output/sixfactor.norms.test2.fit", label = "Norms 6-Factor L2",
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)
...
```{r norms-hierarchical-testc, eval = RUN_MODEL}
hierarchical.model.testc <- 
paste0(' 
Level: 1
  D_self =~ dhf_norm_1+dhf_norm_2+dhf_norm_3+dhf_norm_4+dhf_norm_5
...
```{r norms-sixfactor-testc, eval = RUN_MODEL}
sixfactor.model.testc <- 
paste0(' 
Level: 1
...
sixfactor.norms.testc.fit <- run_lavaan(cfa,model = sixfactor.model.testc,
                    data = dt4,
                    cluster = "country_res_full",
                    # options to consider
                    #optim.method = "em", 
                    ...
                    #em.fx.tol = 1e-08, 
                    #em.dx.tol = 1e-04,
                    save_path = "output/sixfactor.norms.testc.fit"
)
```
...
lavaan_summary(sixfactor.norms.testc.fit, rds_path = "output/sixfactor.norms.testc.fit", label = "Norms 6-Factor Combined",
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)
...
```{r norms-threefactor-testc, eval = RUN_MODEL}
threefactor.model.testc <- 
paste0(' 
Level: 1
...
threefactor.norms.testc.fit <- run_lavaan(cfa,model = threefactor.model.testc,
                    data = dt4,
                    cluster = "country_res_full",
                    # options to consider
                    #optim.method = "em", 
                    ...
                    #em.fx.tol = 1e-08, 
                    #em.dx.tol = 1e-04,
                    save_path = "output/threefactor.norms.testc.fit"
)
```
...
lavaan_summary(threefactor.norms.testc.fit, rds_path = "output/threefactor.norms.testc.fit", label = "Norms 3-Factor Combined",
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)
...
```{r norms-six3factor-testc, eval = RUN_MODEL}
six3factor.model.testc <- 
paste0(' 
Level: 1
...
six3factor.norms.testc.fit <- run_lavaan(cfa,model = six3factor.model.testc,
                    data = dt4,
                    cluster = "country_res_full",
                    # options to consider
                    #optim.method = "em", 
                    ...
                    #em.fx.tol = 1e-08, 
                    #em.dx.tol = 1e-04,
                    save_path = "output/six3factor.norms.testc.fit"
)
```
...
lavaan_summary(six3factor.norms.testc.fit, rds_path = "output/six3factor.norms.testc.fit", label = "Norms 6+3-Factor Combined",
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)
...
```{r norms-six3factor-testc-no13, eval = RUN_MODEL}
...
six3factor.norms.testc.no13.fit <- run_lavaan(cfa,model = six3factor.model.testc.no13,
                    data = dt4,
                    cluster = "country_res_full",
                    # options to consider
                    #optim.method = "em", 
                    ...
                    #em.fx.tol = 1e-08, 
                    #em.dx.tol = 1e-04,
                    save_path = "output/six3factor.norms.testc.no13.fit"
)
```
...
lavaan_summary(six3factor.norms.testc.no13.fit, rds_path = "output/six3factor.norms.testc.no13.fit", label = "Norms 6+3-Factor Combined no13",
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)
...
```{r norms-six3factor-testc-no20, eval = RUN_MODEL}
six3factor.model.testc.no20 <- 
paste0(' 
Level: 1
...
six3factor.norms.testc.no20.fit <- run_lavaan(cfa,model = six3factor.model.testc.no20,
                    data = dt4,
                    cluster = "country_res_full",
                    # options to consider
                    #optim.method = "em", 
                    ...
                    #em.fx.tol = 1e-08, 
                    #em.dx.tol = 1e-04,
                    save_path = "output/six3factor.norms.testc.no20.fit"
)
```
...
lavaan_summary(six3factor.norms.testc.no20.fit, rds_path = "output/six3factor.norms.testc.no20.fit", label = "Norms 6+3-Factor Combined no13+20",
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)
...
endorse.model.configural.fit <- run_lavaan(cfa,
  ...
  missing = "fiml",
                    save_path = "output/endorse.model.configural.fit"
)
```
...
lavaan_summary(endorse.model.configural.fit, rds_path = "output/endorse.model.configural.fit", label = "MGCFA Endorsement Configural",
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)
...
```{r mgcfa-endorse-align, eval = RUN_MODEL}
...
norms.model.configural.fit <- run_lavaan(cfa,
  ...
  std.lv = FALSE,      # <- critical to match Mplus
  missing = "fiml",
                    save_path = "output/norms.model.configural.fit"
)
```
...
lavaan_summary(norms.model.configural.fit, rds_path = "output/norms.model.configural.fit", label = "MGCFA Norms Configural",
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)
...
```{r mgcfa-norms-align, eval = RUN_MODEL}
...
#   summarize(
#     across(
#       c(D_self_end, D_other_end, H_self_end, H_other_end, F_self_end, F_other_end,
#         D_end, H_end, F_end,
#         D_self_endc, D_other_endc, H_self_endc, H_other_endc, F_self_endc, F_other_endc,
#         D_endc, H_endc, F_endc),
#       list(
#         mean = ~mean(.x, na.rm = TRUE),
#         sd   = ~sd(.x, na.rm = TRUE)
#       ),
#       .names = "{.col}_{.fn}_country"
#     )
#   )

write.csv(country_endors, "output/country_endorsement.csv")
...
#   summarize(
#     across(
#       c(D_self_norm, D_other_norm, H_self_norm, H_other_norm, F_self_norm, F_other_norm,
#         D_norm, H_norm, F_norm,
#         D_self_normc, D_other_normc, H_self_normc, H_other_normc, F_self_normc, F_other_normc,
#         D_normc, H_normc, F_normc),
#       list(
#         mean = ~mean(.x, na.rm = TRUE),
#         sd   = ~sd(.x, na.rm = TRUE)
#       ),
...
write.csv(country_norm, "output/country_norms.csv")
...
ASK LUCY ABOUT THIS SECTION 
- A few missing variables 
- The Openness_to_Change_mean_country, Conservation_mean_country, Self_Enhancement_mean_country ... no code found on how these were calculated
- Also need to figure out where the other variables are 
...
lavaan_summary(provocation.fit.6f, label = "H5 Provocation 6-Factor", 
        standardized = TRUE,
        fit.measures = TRUE,
        rsquare = TRUE)
...
lavaan_summary(provocation.fit.3f, label = "H5 Provocation 3-Factor", 
        standardized = TRUE,
        fit.measures = TRUE,
        rsquare = TRUE)
