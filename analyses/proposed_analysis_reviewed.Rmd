...
This most likely does the job, but I would ensure not missing a parameter by constructing the model just from the variable names.
...
* Here, I would not define the column numbers, in the case these would not match, for some unexpected reason. If the variable names should match, obtain the column number by variable name instead.

* This was also present at other parts below, but I did not edit all of them
...
* Here I would also not use the column numbers for the same reason. To dplyr-pipe, you can supply sequential variable names directly.
...
dt <- dt %>%
  mutate_at(vars(DHF_endorsement_1:DHF_Norms_30),~dplyr::recode(., "Strongly disagree"=1, 
                                       "Disagree"=2,"Somewhat disagree"=3,
                                       "Somewhat agree"=4,"Agree"=5,
                                       "Strongly agree"=6))
...
* This does not work for some reason. I understood this should be from 1 to 7. With the original code, 1 and 7 responses were coded as NA. I tried to make an alternative approach.
...
* This did not work as supposed. All the odd numbers become missing. I tried to use case_when here as well.
...
dt <- dt %>%
  mutate_at(vars(`Self-construal_1`:`Self-construal_24`),~dplyr::case_when(
...
* From what I understood, this runs smoothly and how it should run. But, a brief description here could help. Describe what part is made long format and for what purpose. A brief comment above each line of code, or so.

* Perhaps do not name data frames as "c", because it is also a function in R?
...
* Runs smoothly, but a brief comment would help to understand.
...
* Not working for me, not sure if should work. The problem seems to be that country was a character variable
...
* Runs but non-convergence issues. I understood these as resulting from only two level-2 clusters in the pilot data?

* Model seems to be correctly defined
...
* Same as above: Model seems to be correctly defined. Applies to all following CFA-models, not commented.
...
* Code seems to run smoothly
* The factors are here defined as orthogonal. Shouldn't these be correlated as in Figure 1? Or is this somehow necessary for this procedure?
...
* This did not run. "prosociality" could not be found.
