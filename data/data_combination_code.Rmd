...
* Some participants have a "live" status, should they be included or only the "complete" participants?
...
africa_full <- read.csv("raw_data/Africa Long Life/ALLS_W6_anon.csv", na.strings = c(" ", ""))  
```
...
**Priya said: Once all Qualtrics data is in, get list of all the Q_language codes for me and I’ll tell you which ones need to be deleted**
...
* Deleting all rows which do **not** a number in the language code (no number means participants saw one of the “base” versions without customised information for their lab. Can’t be sure they consented properly or saw the correct information).
...
dhf_all %>% 
  group_by(country_res, country_res_full, Country, Country_full) %>% 
  count()

dhf_all <- dhf_all %>%
  left_join(country_names, by = "Country", suffix = c("", "_lookup")) %>%
  mutate(
    Country_full = if_else(
      is.na(Country_full),        # only replace if current value is NA
      Country_full_lookup,        # take the one from lookup_df
      Country_full                # otherwise keep the existing one
    )
  ) %>%
  select(-Country_full_lookup)     # remove the temporary column

View(dhf_all %>%
  filter(is.na(Country_full)) %>%
  select(Country, Country_full))
```

ABOVE HERE IS WHERE I'M GETTING THE ISSUE: I can't work out why there are still 7 cases where there's no Country_full because I thought I fixed it. Also, note, haven't run from here onwards because I'm waiting for the new country-level dataset.
...
country_metadata <- read_csv("codebooks/countries_data.csv", show_col_types = FALSE) %>%
  rename(Country_full = dhf_country_name) %>%
  select(-country_name,
         -iso3c) 
...
* => will need to make this match, but I am leaving this to the analysis team as it will be easier to make these decision when looking at the data in more detail. 
...
```{r}
write.csv(dhf_all, "processed_data/dhf_data_final.csv")
```
