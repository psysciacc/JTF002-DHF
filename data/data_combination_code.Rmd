...
* country res - this was at the bottom, needed to be up here to be in the correct chain of events 

```{r}
# this code fixes the issue with Palestine not matching the numbers in qualtrics
dhf_all <- dhf_all %>%
...
dhf_all %>% 
  filter(country_res_full == "Rwanda")
```
...
# add in additional country variables
country_metadata2 <- read_csv("codebooks/additional_country_variables_merged.csv") %>%
  rename(country_res_full = dbf_country) %>% 
  select(-iso3c)

dhf_all <- dhf_all %>% 
  left_join(country_metadata2)

# View(dhf_all %>% select(country_res_full,W_2000:RuleOfLaw_2024) %>% filter(is.na(W_2000)))
# ani if you want to check the missing data 
```
