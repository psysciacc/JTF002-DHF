...
Individual endorsement: ratings on DHF for how much the norms endorses each item
...
Sample-level: using an average score by the country to represent the variable

Level 1: [within] analysis at the norms data points in a multilevel analysis 

Level 2: [cluster] the data points nested or clustered by the indicated variable (i.e., basically the average score for each item across the cluster variable as the level of analysis)
...
In each structural model, we may find the following issues:
  ...
  - Heywood cases:
    - Too high correlations: we will consider combining factors when correlations betwen latent variables are too high if they theoretically make sense.  
  ...
  - Non-convergence: 
    ...
    - For bifactor models, we will consider setting all exogenous only correlations to 0 - rather than the proposed model below - to ensure identification. 
...
H1A: A bifactor model with three general factors of dignity, honor, and face, and six factors representing the self-concern and other-concern facet of each, will appropriately describe the data.
...
Factor structure: Multilevel confirmatory factor analysis (CFA), specifying a bifactor model with three correlated latent factors representing dignity, honor, and face, and six latent factors representing the self-concern and other-concern facets of each. The self- and other-facets of the three constructs will correlate. 
...
k <- 618 # number of estimated parameters 
...
### Bifactor Individual Test 1

```{r individual-bifactor-test1, eval = F}
bifactor.model.test1 <- 
paste0(' 
Level: 1
...
bifactor.individual.test1.fit <- cfa(model = bifactor.model.test1,
                    data = DF_individual,
                    cluster = "country"
                    # options to consider
                    #optim.method = "em", 
...
summary(bifactor.individual.test1.fit,
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)

bifactor.individual.test1.fitindices <- tidy(fitmeasures(bifactor.individual.test1.fit))
bifactor.individual.test1.params <- tidy(bifactor.individual.test1.fit)
```

### Bifactor Individual Test 2

```{r individual-bifactor-test2, eval = F}
bifactor.model.test2 <- 
paste0(' 
Level: 1', 
corr_error, 
'
...
bifactor.individual.test2.fit <- cfa(model = bifactor.model.test2,
                    data = DF_individual,
                    cluster = "country"
                    # options to consider
                    #optim.method = "em", 
...
summary(bifactor.individual.test2.fit,
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)

bifactor.individual.test2.fitindices <- tidy(fitmeasures(bifactor.individual.test2.fit))
bifactor.individual.test2.params <- tidy(bifactor.individual.test2.fit)
```

## H1A Norms

### Bifactor Norms Test 1

```{r norms-bifactor-test1, eval = F}
bifactor.model.test1 <- 
paste0(' 
Level: 1
  D_self =~ D_self1+D_self2+D_self3+D_self4+D_self5
...
bifactor.norms.test1.fit <- cfa(model = bifactor.model.test1,
                    data = DF_norms,
                    cluster = "country"
                    # options to consider
                    #optim.method = "em", 
...
summary(bifactor.norms.test1.fit,
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)

bifactor.norms.test1.fitindices <- tidy(fitmeasures(bifactor.norms.test1.fit))
bifactor.norms.test1.params <- tidy(bifactor.norms.test1.fit)
```

### Bifactor Norms Test 2

```{r norms-bifactor-test2, eval = F}
bifactor.model.test2 <- 
paste0(' 
Level: 1', 
corr_error, 
...
bifactor.norms.test2.fit <- cfa(model = bifactor.model.test2,
                    data = DF_norms,
                    cluster = "country"
                    # options to consider
                    #optim.method = "em", 
...
summary(bifactor.norms.test2.fit,
        fit.measures = TRUE,
        rsquare = TRUE,
        standardized = TRUE)

bifactor.norms.test2.fitindices <- tidy(fitmeasures(bifactor.norms.test2.fit))
bifactor.norms.test2.params <- tidy(bifactor.norms.test2.fit)
```
...
H1b-H1c: The bifactor model will fit the data better than the more restrictive correlated 6-factor model (H1b), or a 3-factor model (H1c).

*Analysis Plan*: 

Individual endorsement and cultural norms will be tested separately. For each model, country will be used as a cluster variable. 

Factor structure: Multilevel CFA using a six-factor correlated model of each latent variable (DHF) separated by self/other distinction, and three-factor correlated model of the DHF latent variables will be analyzed. The same item removal and correlated residuals will be retained from H1A. 
...
All these criteria will be considered together, and if the majority of them are fulfilled, the model will be considered significantly better than the alternative. If models are not significantly different from each other, the theoretically hypothesized bifactor model will be preferred.

## Power

```{r power-comparison}
k2 <- 570 # number of parameters in 6 factor
k3 <- 558 # number of parameters in 3 factor
...
bifactor.individual.test1.fitindices
bifactor.individual.test2.fitindices
...
# for example 
bifactor.individual.test1.fit

# examine modification indices
modificationindices(bifactor.individual.test1.fit)
...
bifactor.norms.test1.fitindices
bifactor.norms.test2.fitindices
...
# for example 
bifactor.norms.test1.fit

# examine modification indices
modificationindices(bifactor.norms.test1.fit)
...
# temp <- cfa(model = bifactor.align.model,
#             data = DF_individual,
#             orthogonal = TRUE)
# summary(temp, standardized = TRUE)

# calculate MGCFA
bifactor.mgcfa.fit <- mgcfa(
  model = bifactor.align.model,
  data = DF_individual, 
  group = "country",
...
summary(bifactor.mgcfa.fit$model_overall)
summary(bifactor.mgcfa.fit$model_configural)
bifactor.mgcfa.fit$model_fit

# extract item parameters separate group analyses
ipars <- lavaan::parameterEstimates(bifactor.mgcfa.fit$model_configural)
...
Ng <-  unlist(bifactor.mgcfa.fit$model_configural@SampleStats@nobs)
